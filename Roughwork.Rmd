...
poldata <- read.csv("Political spectrum.csv", header=TRUE)
poldat <- poldata[,-1]
...
library(boot)
library(fpc)
library(ggcorrplot)  # For better correlation matrix visualization
library(GGally)
library(dplyr)
library(ggdensity)
...
Also look at correlation between variables now that the missing values have been imputed.

```{r}
# Compute the correlation matrix
cor_matrix <- cor(polNumImp)
...
sort(abs((pca_result$rotation)[,1]), decreasing = T)
```
...
```{r}
d <- dist(pol_scaled)  # Euclidean distance by default

# Average linkage
# hc_result <- hclust(d, method = "average")
hc_result <- hclust(d, method = "ward.D")

# Plot the dendrogram
plot(hc_result)
rect.hclust(hc_result, k = 4)  # Highlight clusters; adjust 'k' as needed
...
nclust0 <- nselectboot(data = pol_scaled, B=100, clustermethod = hclustCBI, 
            method = "ward.D", krange = 3:10)

nclust0$kopt
nclust0$stabk #get the instability values

cl3 <- clusterboot(data = pol_scaled, B=200, bootmethod = c("boot"),
                      clustermethod = hclustCBI, method = "ward.D", k = 3, seed=777)

cl4 <- clusterboot(data = pol_scaled, B=200, bootmethod = c("boot"),
                      clustermethod = hclustCBI, method = "ward.D", k = 4, seed=777)

cl5 <- clusterboot(data = pol_scaled, B=200, bootmethod = c("boot"),
                      clustermethod = hclustCBI, method = "ward.D", k = 5, seed=777)
cl6 <- clusterboot(data = pol_scaled, B=200, bootmethod = c("boot"),
                      clustermethod = hclustCBI, method = "ward.D", k = 6, seed=777)
...
# Image saved and not printed here, because it takes a lot of time to render

# fviz_dend(hc_result, k = 4, # Cut into 4 clusters
#           rect = TRUE,
#           show_labels = FALSE)

# scaled data with cluster assignments
clusterdat <- cbind(pol_scaled,clustergroup= as.factor(cl4$result$partition))
# original data with cluster assignments
polclust <- cbind(polNumImp,clustergroup= as.factor(cl4$result$partition))

# Select a subset of predictors to visualize (adjust as needed)
selected_predictors <- polclust[, c("Environment", 
                                      "Economic", "wantPossible", "ownAll",
                                      "Change", "lawsPeople", "clustergroup")]

# Create pairwise plots
# ggpairs(selected_predictors, aes(color = as.factor(clustergroup)))


```

Now we rather use the unscaled data and the summary statistics for the interpretation, otherwise it is difficult to see whether a cluster is really left or right.

```{r}
ggplot(data = polclust, aes(x = Economic, y = wantPossible, fill = clustergroup)) +
  geom_hdr(xlim = c(-10, 110), ylim = c(-10, 110),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of Economic vs How people live",
       x = "Economic",
       y = "Want - Possible",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
  theme_minimal() +  # Use a minimal theme
  scale_color_brewer(palette = "Set2") +  # Color for points
  scale_fill_brewer(palette = "Set2")  # Color for shading

ggplot(data = polclust, aes(x = Economic, y = lawsPeople, fill = clustergroup)) +
  geom_hdr(xlim = c(-10, 110), ylim = c(-10, 110),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of Economic vs Better laws/people",
       x = "Economic",
       y = "Better laws vs better people",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
  theme_minimal() +  # Use a minimal theme
  scale_color_brewer(palette = "Set2") +  # Color for points
...
ggplot(data = polclust, aes(x = Environment, y = wantPossible, fill = clustergroup)) +
  geom_hdr(xlim = c(-10, 110), ylim = c(-10, 110),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of Environment vs How people live",
       x = "Environment",
       y = "Want - Possible",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
ggplot(data = polclust, aes(x = Change, y = wantPossible, fill = clustergroup)) +
  geom_hdr(probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of Change vs How people live",
       x = "Change",
       y = "Want - Possible",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
ggplot(data = polclust, aes(x = ownAll, y = wantPossible, fill = clustergroup)) +
  geom_hdr(xlim = c(-10, 110), ylim = c(-10, 110),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of ownAll vs How people live",
       x = "ownAll",
       y = "Want - Possible",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
ggplot(data = polclust, aes(x = LGBT, y = Environment, fill = clustergroup)) +
  geom_hdr(xlim = c(-2, 10), ylim = c(-10, 110),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of LGBT vs Environment",
       x = "LGBT",
       y = "Environment",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
ggplot(data = polclust, aes(x = LGBT, y = mRight, fill = clustergroup)) +
  geom_hdr(xlim = c(-2, 10), ylim = c(-2, 10),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of LGBT vs mRight",
       x = "LGBT",
       y = "mRight",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
ggplot(data = polclust, aes(x = mLeft, y = mRight, fill = clustergroup)) +
  geom_hdr(xlim = c(-2, 10), ylim = c(-2, 10),
           probs = c(0.75,0.5, 0.25)) +
  geom_point(shape=21) +  # Points with some transparency
  labs(title = "Scatter Plot of mLeft vs mRight",
       x = "mLeft",
       y = "mRight",
       color = "Cluster Group",  # For points
       fill = "Cluster Group") +  # For density polygons
...
Get the summary statistics for the clusters:

```{r}
# Get the average for each predictor variable by cluster
clusterdat_summary <- polclust %>%
  group_by(clustergroup) %>%
  summarise(across(c(Economic, Environment, wantPossible, unityDifferences, 
                     lawsPeople, ownAll, pCongress, pBusiness, tCongress, 
                     tBusiness, Change, LGBT, mLeft, mRight), 
                   \(x) mean(x, na.rm = TRUE)))

# View the result
print(clusterdat_summary)
```
